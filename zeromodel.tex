...
\tikzset{capacity/.style={domain=0:60, draw=red, thick}}
	...
	\end{axis}
	\end{tikzpicture}
	\caption{The parabola represents the collected revenue rate curve for a given fare without any constraint of deployed fleet capacity. The lines labeled $n_i$ represent the revenue rates upper limits given by the fleet capacity $a(n_i)$. Points labeled \msn{i} label the optimal fare for the given fleet size $n_i$, i.e. $\ms_i=\argmax\limits_{m\in(\frac{\cd}{2},\cd); n=n_i} \{\min(a(n_i),q(m))\}$.}
	\label{gr:revenue}
\end{figure}
...
\pgfplotsset{width=0.95\linewidth, height=0.7\linewidth,compat=1.15}
	\begin{tikzpicture}
		\begin{axis}[view={120}{40},
			...
			{\edef\temp{\noexpand\addplot3 [thick, red, domain y=\limitA:\limitB] ( {\n}, {y}, {\slope*(y-\limitA)} ) node (\nodename) [dot=6pt,draw=red,pos=1]{};}\temp}
			}
